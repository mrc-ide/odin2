...
## Depending on how mcstate develops, we might move this file into
## that package.
differentiate <- function(expr, name) {
  if (is.numeric(expr)) {
    ...
    if (fn %in% c("Normal", "Poisson", "Uniform")) {
      browser()
    }
...
## TODO: this is going to need distribution-variant support, once we have it
log_density <- function(distribution, args) {
  switch(
    distribution,
    ...
    normal = substitute(
      ...
      list(x = args[[1]], mu = args[[2]], sd = args[[3]])),
    poisson = substitute(
      x * log(lambda) - lambda - lfactorial(x),
      list(x = args[[1]], lambda = args[[2]])),
    uniform = substitute(
      if (x < a || x > b) -Inf else -log(b - a),
      list(x = args[[1]], a = args[[2]], b = args[[3]])),
    cli::cli_abort("Unsupported distribution '{distribution}'"))
}
