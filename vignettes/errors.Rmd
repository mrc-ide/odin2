...
Invalid special function used on the right hand side.  Some odin functions can only be used on the lhs; this include `initial`, `update` and `deriv`.  So you cannot write this:
...
which would not be valid in R.  Always provide an `else` branch.
