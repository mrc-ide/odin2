...
This is new and very experimental, you can ask odin to drop you into a debugger (implemented on top of R's `browser()` to explore the values of your variables at some point within a model run.  To do this, add a call to `debug()` somewhere within your odin code and recompile.

The `debug` argument accepts arguments
...
The location of the call to `debug()` does not matter; it will be activated at the **end** of the phase.  The condition here might be something we cook up to look at what happens as the number of individuals in the infected class starts tailing off at the end of the simulation.
...
However, when you run the sytem you will pause part way through evaluation:

```r
dust_system_run_to_time(sys, 200)
#> ℹ dust debug: see `?dust_debug()` for help
...
and press `c` to continue to the end of your simulation.  You will need to manually reenable the debugger by running

```r
dust_debug_enabled(TRUE)
```

if you want to use the debugger again though.
