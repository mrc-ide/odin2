...
`odin2` supports many functions that you'd expect to see for constructing dynamocal models.  These include most common mathematical operations and some that are quite obscure.  There is a special syntax for working with arrays, which is described in some detail below.  The support for stochastic models and comparison to data come from `monty`.
...
We support distribution funtions in two places:
...
will assign `a` to a draw from the strandard normal distibution.  You cannot use these functions from continuous time models.  You cannot sample from stochastic functions in a continuous time (ODE) model.
...
* `Normal` -- the [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution) mean, sd
