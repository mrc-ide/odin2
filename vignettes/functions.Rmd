...
* `round` -- **Round a number**: round a number to the nearest integer (e.g., `round(1.23)` &rarr; `1`). Note that this uses the C++ convention that half rounds away from zero (e.g. `round(2.5)` &rarr; `3`) in contrast to the R convention that half rounds to even (in R, `round(2.5)` &rarr; `2`)
...
* `factorial` -- **Factorial function** (e.g., `factorial(10)` &rarr; `3628800`)
* `lfactorial` -- **Log factorial function** (e.g., `lfactorial(10)` &rarr; `15.10441`)
* `choose` -- **Binomial coefficient function** (e.g. `choose(7, 3)` &rarr; `35`)
* `lchoose` -- **Log binomial coefficient function** (e.g. `lchoose(7, 3)` &rarr; `3.555348`)
* `erf` -- **Error function** (e.g., `erf(1)` &rarr; `0.8427008`)
* `erfc` -- **Complementary error function** (e.g., `erfc(1)` &rarr; `0.1572992`)
