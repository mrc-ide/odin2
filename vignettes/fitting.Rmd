...
We need to develop nice tools for working with the outputs of the sampler, but for now bear with some grubby base R maniulation.
...
Here, you can see the 1000 samples from the deterministic model (in blue) overlaied on top of the samples from the stochastic model (in grey):
...
The estimated parameters here look overal shifted higher in the deterministic model, and the correlation between the parameters stronger.  However, if we had no idea about what "good" parameters might be, this can get us into the approximately right location.
