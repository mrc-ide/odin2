...
```{r}
dust2::dust_likelihood_run(filter, list(beta = 0.2, gamma = 0.1), save_history = TRUE)
```

You can access these with `dust2::dust_likelihood_last_history()`:

```{r}
h <- dust2::dust_likelihood_last_history(filter)
```
...
```{r}
filter <- dust2::dust_likelihood_create(sir(), 0, data, n_particles = 1000)
likelihood <- dust2::dust_likelihood_monty(filter, sir_packer)
...
```{r}
likelihood <- dust2::dust_filter_monty(likelihood, sir_packer)
likelihood
```
